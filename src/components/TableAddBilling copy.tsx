...
import { useGetProductsByUserIdQuery } from "../slices/productApiSlice"; 
...
const TableAddBilling: FunctionComponent<TableConfigProps> = ({
  ...
  const [dateIssue, setDateIssue] = useState(
    itemToUpdate && typevalue === "View"
      ? formatDate(itemToUpdate.dateIssue)
      : ""
  );
  ...
  const [invalidationKey, setInvalidationKey] = useState<number>(0); 

  const [editableAmount, setEditableAmount] = useState("");
  const [productAmounts, setProductAmounts] = useState<{
    [productId: number]: number;
  }>({});
  const [searchResultsUpdated, setSearchResultsUpdated] = useState<
    Array<{
      invoiceID: number;
      invoiceType: string;
      productId: number;
  ...
  const { data: dataResponseRegisters, isLoading } =
  ...
  const { data: dataProductsInvoicesRegisters, refetch: customRefetch } =
  ...
  const {
    data: generateIdData,
    error: generateIdError,
    refetch: generateIdRefetch,
  ...
  useEffect(() => {
    if (typevalue === "View") {
      ...
      if (dataProductsInvoicesRegisters) {
        ...
        const dataInvoiceIdProducts = dataProductsInvoicesRegisters.filter(
          (item: { invoiceID: any }) => item.invoiceID === invoiceID
        );
      ...
      for (const product of searchResultsUpdated) {
        ...
        if (typevalue === "Sales") {
          multiplicationResult =
            (product.price + product.price * (product.utility / 100)) *
            product.amount;
  ...
  const handleConfirmAll = async () => {
    try {
      for (const originalProduct of searchResults) {
        ...
        product.amount = productAmounts[product.productId]; 
        product.dateIssue = dateIssue;

        if (typevalue === "Purchase") {
          const productToUpdate = { ...originalProduct };

          productToUpdate.amount = originalProduct.amount + product.amount;

          const response = await updateProductAmount({
      ...
      setSearchResults([]);
      toast.success(
        "Se ha emitido la factura correctamente con los datos proporcionados"
      );
    } catch (error) {
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            {(typevalue === "Purchase" ||
              (itemToUpdate &&
                typevalue === "View" &&
                itemToUpdate.invoiceType === "Purchase")) && (
              <TextField
                label="Provider"
                variant="standard"
                type="text"
          ...
          <Grid item xs={6}>
            {(typevalue === "Sales" ||
              (itemToUpdate &&
                typevalue === "View" &&
                itemToUpdate.invoiceType === "Sales")) && (
              <TextField
                label="Customer"
                variant="standard"
                type="text"
                ...
                : searchResults.map((product) => (
                    <TableRow key={product.productId}>
                      ...
                      <TableCell>
                        <IconButton
                          onClick={() =>
                            handleDeleteFromList(product.productId)
                          }
                        >
                          <DeleteIcon />
                        </IconButton>
...
//   invoiceID: number;
//   invoiceType: string;
//   productId: string; // Cambiar a string solo aquí
//   name: string;
//   description: string;
//   utility: number;
//   price: number;
...
//         // product.utility = product.utility;
//         console.log("---Producto Actualizado :D");
//         console.log(product);

//         if (typevalue === 'Purchase') {
//           const productToUpdate: ProductToUpdate = { 
//             ...originalProduct, 
//             amount: (originalProduct.amount + product.amount),
//             productId: String(originalProduct.productId), // Convertimos solo aquí
//           };
...
//         // registro productinvoice (invoice-product)
//         const response = await addProductInvoiceMutation({
//           registro: product,
//           token: token,
//         });
